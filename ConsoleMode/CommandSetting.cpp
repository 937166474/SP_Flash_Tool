...
void CommandSetting::loadXML(const QDomElement &xml_node)
{
    ...
    while (!command_node.isNull()) {
        ...
        //add cmd before reboot cmd, which need to be the last one.
        if (command_name == REBOOT_CMD_NAME_VAL) {
            this->addRebootToATMModeCmd();
            if (OptionIniFileMgr::getInstance()->getBoot_to_fastmode()) {
                addCmdWrapper(this->createFastBootModeCmdWrapper());
            }
        }
...
cmd_wrapper_ptr CommandSetting::createFastBootModeCmdWrapper() const
{
    return std::make_shared<CmdWrapper>(std::make_shared<FastBootModeCmdXML>());
}
...
cmd_wrapper_ptr CommandSetting::createRSCCmdWrapper() const
{
    ...
    if (m_cmd_args && m_cmd_args->getCurrentMode() == ConsoleModeCurrentMode::CmdArgumentMode) {
