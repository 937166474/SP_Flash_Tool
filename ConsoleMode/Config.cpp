...
{
    QDomElement root_node = this->getRootElement(xml_file_name);
    if (root_node.isNull()) {
        return ;
    }
...
std::shared_ptr<LogInfoSetting> Config::getLogInfoSetting(const QString &config_xml_name)
{
    QDomElement root_node = getRootElement(config_xml_name);
    return GeneralSetting::getLogInfoSetting(root_node);
}
...
QDomElement Config::getRootElement(const QString &xml_file_name)
{
    if (xml_file_name.isEmpty()) {
    ...
    if (root_node.attribute(CONFIG_XML_ROOT_NODE_ATTR_NAME) != CONFIG_XML_ROOT_NODE_ATTR_VAL) {
        LOGE("NOT support config.xml of version %s, please update tool or contact owner!\n",
             qPrintable(root_node.attribute(CONFIG_XML_ROOT_NODE_ATTR_NAME)));
        return QDomElement();
    }
    return root_node;
}
