...
std::shared_ptr<LogInfoSetting> GeneralSetting::getLogInfoSetting(const QDomElement &root_element)
{
    std::shared_ptr<LogInfoSetting> sp_loginfo(nullptr);
    if (root_element.isNull()) {
        return sp_loginfo;
    }
    QDomElement general_node = root_element.firstChildElement("general");
    if (general_node.isNull()) {
        return sp_loginfo;
    }
    sp_loginfo = std::make_shared<LogInfoSetting>();
    sp_loginfo->loadXML(general_node);
    return sp_loginfo;
}
