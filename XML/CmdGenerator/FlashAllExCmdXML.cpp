#include "FlashAllExCmdXML.h"
#include <QDir>

FlashAllExCmdXML::FlashAllExCmdXML():
    ICmdXMLGenerator(FLASH_ALL_EX_CMD_VER_VAL, FLASH_ALL_EX_CMD_NAME_VAL, DA_NODE_NAME),
...
void FlashAllExCmdXML::loadXML(const QDomElement &xml_node)
{
...
void FlashAllExCmdXML::saveXML(QDomDocument &xml_doc, QDomElement &xml_node) const
{
    QDomElement flash_all_ex_node = xml_doc.createElement(EXTRACT_CMD_NAME_ONLY(FLASH_ALL_EX_CMD_NAME_VAL));
    flash_all_ex_node.setAttribute(VERSION_NODE_NAME, FLASH_ALL_EX_CMD_VER_VAL);
    xml_node.appendChild(flash_all_ex_node);
    flash_all_ex_node.setAttribute(QStringLiteral("path_separator"), m_path_separator);
...
void FlashAllExCmdXML::setPathSeparator(const QString &path_separator)
{
...
*/
void FlashAllExCmdXML::fillArgNode(QDomElement * const arg_node)
{
...
void FlashAllExCmdXML::setFileType(FILE_TYPE file_type)
{
    m_source_file_assist->setFileType(file_type);
}

void FlashAllExCmdXML::setFileName(const QString &file_name)
{
    m_source_file_assist->setFileName(file_name);
}

void FlashAllExCmdXML::setMemInfo(const QString &start_address, const QString &length)
{
    m_source_file_assist->setStartAddress(start_address);
    m_source_file_assist->setLength(length);
}

void FlashAllExCmdXML::setDLPartitionList(const DownloadPartitionInfoList &dl_partition_list)
{
