...
bool ScatterXMLParser::parseStorageTypeNode(const QDomElement &storage_type_node)
{
    ...
    QSet<QString> partition_name_set;
    ...
    while (!partition_element.isNull()) {
        ...
        parse_success = parsePartitionIndexNode(partition_list, partition_name_set, partition_element);
        if (!parse_success) {
            break;
        }
        partition_element = partition_element.nextSiblingElement();
                                               ...
                                               const QDomElement &partition_index_node)
{
    ...
    QString partition_name = part_name_node.text().trimmed();
    if (part_name_set.contains(partition_name)) {
        m_error_msg = QStringLiteral("The scatter file contains duplicated partition name[%1].\n"
                                     "Please select a valid load!").arg(partition_name);
        return false;
    }
    part_name_set.insert(partition_name);
    ...
    if (is_download)
    {
        ...
        partition_info.setPartition_name(partition_name);
